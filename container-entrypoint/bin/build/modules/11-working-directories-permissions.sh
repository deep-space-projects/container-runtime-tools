...
load_module_implementation "$SCRIPT_DIR" "working-directories-permissions"
...
module() {
    ...
    for working_dir in "${working_dirs[@]}"; do
        ...
        if [[ $set_permissions_result -ne 0  ]]; then
            ops handle-quite "setup working directory permissions for $working_dir" \
    ...
    if [[ ${#processed_dirs[@]} -gt 0 ]]; then
        ...
        if ! verify_working_directories_permissions "${processed_dirs[@]}"; then
            ops handle-quite "verify working directories permissions" \
