...
check_volume_configuration() {
    ...
    tlog info "[DRY RUN] Would process CONTAINER_VOLUME_DIRS: $CONTAINER_VOLUME_DIRS"
...
parse_volume_directories() {
    tlog info "[DRY RUN] Would parse volume directories from CONTAINER_VOLUME_DIRS"

    # Реально парсим для демонстрации
    IFS=',' read -ra VOLUME_DIRS_ARRAY <<< "$CONTAINER_VOLUME_DIRS"
